...
function Results_All({
  ...
  // const [formatted_response , set_formatted_response] = useState('')
  

  // console.log("allData",allData);
const handle_click_json = (Info) =>{
console.log(Info);
  if (Info.response_file_name    == undefined || Info.response_file_name    == null || Info.response_file_name    == "" ){
    console.log("dont have response name");
...
try{
 console.log(Info);
const params = {file_name : Info?.response_file_name }

 const res = await axios.get(`${backEndURL}/results/velociraptor-single-result`,{ params: params});
 
if (res){
   console.log(res.data);
 if (Info?.response_file_name.startsWith("response_velociraptor_")) {
...
// const logoAddress_1 = tool?.logoAddress_1    
...
//  style={{width:"60%" , maxWidth:"60%"}}
 ><p className='font-type-menu  make-underline Color-Grey1 '>Request file name</p></div>
<div className='resource-group-list-item list-item-big   '><p className='font-type-menu  make-underline Color-Grey1 '>Request</p></div>
<div className='resource-group-list-item list-item-big  ' style={{marginRight:"26px"}}><p className='font-type-menu  make-underline Color-Grey1  '>Response</p></div>
...
{  Info?.module_name  === ""   &&    Info?.artifact_name  === ""  &&
 
 <p className='ml-b   font-type-txt   Color-Red   '> Undefined  </p> }
...
{Info?.artifact_name && 
 ...
 <p className="ml-a font-type-very-sml-txt   Color-Grey1  ">+</p>
  <p className="ml-a  font-type-txt   Color-Blue-Glow tagit_type1">{Info?.artifact_name}</p>
...
{Info?.module_name && 
  (<>
   {/* <p class="ml-a  font-type-txt   Color-Blue-Glow tagit_type1">Velociraptor</p>
 <p class="ml-a font-type-very-sml-txt   Color-Grey1  ">+</p> */}
...
//  style={{width:"60%" , maxWidth:"60%" , marginRight:"15px"}}
 >         {Info?.request_file_name }      </p> 
<p className='resource-group-list-item    font-type-txt   Color-Grey1   list-item-big '>{Info?.request_time}</p> 
