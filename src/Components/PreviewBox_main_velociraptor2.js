...
try{
 ...
 console.log(updatedARTS);
  ...
  console.log("artifacts_modules_list",artifacts_modules_list);
...
{/*  read more  */}
    ...
    onClick={()=>handleReadMore(main_headline, main_read_more  , logoAddress ,"Close"  ,iconAddress ,"Big" )}
    
    
    
    ><p className=' font-type-txt' >Read More</p><IconReadMore className="icon-type1 " />  </button>
...
<td  style={{width:"20%"}} ><p className='font-type-menu  Color-White  cutLongLine' >{Info?.headline}                                </p></td>
<td style={{width:"50%"}} > <p className='font-type-txt   Color-Grey1   cutLongLine'  >{box_type === "modules" && Info?.description_short}{box_type === "velociraptor" && Info?.description}                     </p></td>
<td className='' ><div style={{display:"flex", alignItems:"center " }}><p className='font-type-very-sml-txt   Color-Grey1 mr-a'>By:</p><img className='logo-cell  ' style={{ }} src={Info?.logoAddress_1 ? require(`${Info.logoAddress_1}`) : undefined}></img></div></td>
<td><button  className="btn-type3"  
...
export { PreviewBox_velociraptor2,PreviewBoxes_main_modules2};