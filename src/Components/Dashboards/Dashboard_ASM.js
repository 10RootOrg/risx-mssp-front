...
function Dashboard_ASM({show_SideBar,set_show_SideBar,set_unseen_alert_number,set_visblePage}) {
  set_visblePage("dashboard-asm");
    ...
    const { backEndURL  ,all_Resource_Types ,all_artifacts,user_id,    mssp_config_json, front_IP,  front_URL} = useContext(GeneralContext);
  ...
  // dont show sidebar in this page
    ...
    useEffect(() => {
      ...
      const threatHuntingURL = moduleLinks.find(link => link.toolName === "ASM Dashboard")?.toolURL;
