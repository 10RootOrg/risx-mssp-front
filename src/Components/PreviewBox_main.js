...
function PreviewBoxes_main_modules({
  ...
  const change_order_in_db = (info, operator) => {
   ...
   ChangePreviewNumber("Artifact","Artifact", info?.positionNumber , operator ,info);
   
    }

    if (!info?.artifact_id && info?.tool_id) {
      console.log("change_order for - Module");
  ChangePreviewNumber("Module",info?.toolType , info?.positionNumber , operator ,info);
...
const [the_item] =  info.artifact_id != null ?   preview_list.filter((item) => item?.artifact_id === info.artifact_id)  :       preview_list.filter((item) => item?.tool_id === info.tool_id);        
const [the_item_above] =   preview_list.filter((item) => item?.positionNumber  ===  the_item?.positionNumber -1   && item?.BoxType != "Velociraptor");
const [the_item_below] =     preview_list.filter((item) => item?.positionNumber  ===  the_item?.positionNumber +1  && item?.BoxType != "Velociraptor");
...
all_artifacts_and_modules___copy1.map(aaa =>  aaa.artifact_id === the_item.artifact_id ? { ...aaa, positionNumber: the_item.positionNumber -1} : aaa  ) :
all_artifacts_and_modules___copy1.map(aaa =>  aaa.tool_id === the_item.tool_id ? { ...aaa, positionNumber: the_item.positionNumber -1} : aaa  ) 

const all_artifacts_and_modules___copy3 =
info.artifact_id != null ? 
all_artifacts_and_modules___copy2.map(aaa => aaa.artifact_id === the_item_above.artifact_id ? { ...aaa, positionNumber: the_item_above.positionNumber +1} : aaa) :
all_artifacts_and_modules___copy2.map(aaa => aaa.tool_id === the_item_above.tool_id ? { ...aaa, positionNumber: the_item_above.positionNumber +1} : aaa) 
...
info.artifact_id != null ? 
all_artifacts_and_modules___copy2.map(aaa => aaa.artifact_id === the_item_below.artifact_id ? { ...aaa, positionNumber: the_item_below.positionNumber -1} : aaa):
all_artifacts_and_modules___copy2.map(aaa => aaa.tool_id === the_item_below.tool_id ? { ...aaa, positionNumber: the_item_below.positionNumber -1} : aaa)
set_all_artifacts_and_modules(all_artifacts_and_modules___copy3);
...
async function  ShowInUi (Info){
  ...
  try{
   ...
   async function  ChangePreviewNumber (type,subtype,positionNumber,operator ,info ){
    // ChangePositionNumber("Module", info?.positionNumber , operator);
    try{
      const res = await
      axios.put(`${backEndURL}/tools/change-modules-preview-position`,  {
        params: {
          type: type,
...
{/* ---------------------left-box----------------------------- */}
<div className={`${StatusColorClass}  light-bulb-type1 view-on-small-screen_if_vertical`} style={{marginLeft:"auto"}}/>
...
<p  className="font-type-very-sml-txt   Color-Grey1 mr-a mb-b" >By:</p>
<img src={ logoAddress ? require(`${ logoAddress}`) : undefined} alt="velociraptor" maxwidth="180" height="28" position='relative'   className=" mb-b"   /> 
</>}
</div>
...
{/* -----------------------right-box------------------------ */}
  ...
  <div style={{width:"1px" }}/> 
  <div className={`${StatusColorClass}  light-bulb-type1 hide-on-small-screen_if_vertical`} />
</div>
...
<td  className='hide-on-small-screen3  '
...
<td  style={{visibility: box_type ==="velociraptor" && "hidden" ,        minWidth:"115px",  maxWidth:"122px", }}  className= {`${box_type ==="velociraptor" && "spacer-for-non-buttom"} `}>
...
<td className='hide-on-small-screen2' ><div style={{display:"flex", alignItems:"center", width:"auto", minWidth:"95px" ,justifyContent:"center" }}>{/* <IconLastRun /> */}<p className='font-type-very-sml-txt Color-Grey1 cutLongLine' >{Info?.LastRun  &&  format_date_type_a(Info?.LastRun)}</p></div></td>
...
<td >
  ...
  <div 
        ...
        onMouseLeave={() => openIndex === index && setOpenIndex(null)} // Close dropdown on mouse leave
        style={{position:"relative"   }}
 

        //  onMouseEnter={()=>set_download_drop_down(true)}
      >
<button  className= { `${'btn-type5'}`}
style={{padding:0}}
//  onClick={()=>    set_download_drop_down(!download_drop_down)}
 onClick={() => handleDropdownToggle(index)}
 > <IcoSettings className="icon-type1 " /></button>
        ...
        // IcoMenuDown
        // IcoMenuUp
        className={`dropdown-menu ${openIndex === index ? "open" : ""}`}

        style={{position:"absolute" ,right:-5 , zIndex:2 ,top:-4}}
        >

          <p className="font-type-menu ml-c pl-a mr-c mt-b mb-b " style={{minWidth:"150px"}}>{Info?.Tool_name && Info?.Tool_name}{Info?.Toolname &&  Info?.Toolname} Options</p> 
          <div className="Bg-Grey2 mb-b"  style={{ width: "100%", height: "2px", borderRadius: "5px" }} />
    
          <button
            onClick={() => ShowInUi(Info)}
            className="btn-menu ">
            <div className="display-flex"> <p className="font-type-menu ml-c mr-c ">Hide</p> </div>
            <div className="btn-menu-icon-placeholder  ">  <IcoKey      className="btn-menu-icon-placeholder  mr-a  "  style={{ visibility: "" }}  />
            </div>
          </button>
